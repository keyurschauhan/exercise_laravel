    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-10">
                        <h2 class="card-title mb-0">State List</h2>
                    </div>
                    <div class="col-md-2 text-right">
                        <a href="{{ route('state.create') }}" class="btn btn-primary"><i class="fa fa-plus pr-1"></i>Add</a>
                    </div>    
                </div>
                <hr />
                            ...
                            @if(count($state) > 0)
                                @foreach($state as $key => $data)
                                    <tr>
                                        <td>{{ $key+1 }}</td>
                                        <td>{{ $data->name ?? '' }}</td>
                                        <td>@if(!empty($data->code)){{ $data->code ?? '' }} @else - @endif</td>
                                        ...
                                        <td class="text-nowrap d-flex">
                                            <a href="{{ route('state.show',$data->id) }}" data-toggle="tooltip" data-original-title="View"> <i class="fa fa-eye btn btn-info m-r-10"></i> </a>
                                            <a href="{{ route('state.edit',$data->id) }}" data-toggle="tooltip" data-original-title="Edit"> <i class="fa fa-pencil btn btn-success m-r-10"></i> </a>
                                            <form action="{{URL::route('state.destroy',$data->id)}}" method="POST" id="{{$data->id}}">@method('delete')
                                            @csrf
                    ...
                    <div class="col-md-12 paginations">
                        <nav aria-label="Page navigation example">
                            <ul class="pagination justify-content-end mb-0">
                                ...
                                @if(isset($state)){!! $state->render() !!}@endif
